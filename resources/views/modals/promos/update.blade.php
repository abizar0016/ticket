@foreach ($promos as $promo)
    <div id="promoUpdateModal-{{ $promo->id }}"
                ...
                <div class="bg-white dark:bg-gray-800 px-6 pt-6 pb-4 sm:p-6 max-h-[90vh] overflow-y-auto">
                    ...
                    <div class="flex items-start justify-between mb-6">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                <h3 class="text-2xl font-bold text-gray-900 dark:text-white" id="modal-headline">Update Promo Code</h3>
                                <p class="text-indigo-600 dark:text-indigo-400 text-lg mt-1">Update promo code information</p>
                            </div>
                        </div>
                    </div>

                    <form action="{{ route('promos.update', $promo->id) }}" class="ajax-form"
                        data-success="promo updated successfully" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="relative group">
                                    ...
                                    <input type="text" id="promo-{{ $promo->id }}" name="code"
                                        class="w-full px-5 py-4 pl-14 pr-4 text-lg rounded-xl border-2 border-gray-200 dark:border-gray-600 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 transition-all duration-300 shadow-sm group-hover:border-indigo-300 peer outline-none"
                                        placeholder=" " value="{{ old('code', $promo->code) }}" required>
                                    <label for="promo-{{ $promo->id }}"
                                        class="absolute left-14 top-4 px-1 text-gray-500 dark:text-gray-400 text-base transition-all duration-300 transform -translate-y-9 scale-90 bg-white dark:bg-gray-700 rounded peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:-translate-y-9 peer-focus:scale-90 peer-focus:text-indigo-600 dark:peer-focus:text-indigo-400 font-medium">
                                        Promo Code
                                    </label>
                                </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {{-- Ticket Checkbox --}}
                                    <label
                                        class="relative flex items-start p-4 rounded-xl border-2 border-gray-200 hover:border-indigo-300 transition-all duration-300 cursor-pointerhas-[:checked]:border-indigo-400 dark:has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50/50 dark:has-[:checked]:bg-indigo-500/20">
                                        <div class="flex items-center h-5">
                                            <input id="is_ticket-{{ $promo->id }}" name="is_ticket" type="checkbox"
                                                value="1" {{ $promo->is_ticket ? 'checked' : '' }}
                                                class="h-5 w-5 rounded border-2 border-gray-300 dark:border-gray-500 text-indigo-600 focus:ring-indigo-500 transition-all duration-200 peer">
                                        </div>
                                        <div class="ml-3 flex flex-col">
                                            <span
                                                class="text-base font-medium text-gray-900 dark:text-gray-100 peer-checked:text-indigo-700 dark:peer-checked:text-indigo-400">Tickets</span>
                                            <span class="text-sm text-gray-500 dark:text-gray-400 peer-checked:text-indigo-600 dark:peer-checked:text-indigo-300">Apply
                                                discount
                                    ...
                                    <label
                                        ...
                                        <div class="ml-3 flex flex-col">
                                            <span
                                                class="text-base font-medium text-gray-900 dark:text-gray-100 peer-checked:text-indigo-700 dark:peer-checked:text-indigo-400">Merchandise</span>
                                            <span class="text-sm text-gray-500 dark:text-gray-400 peer-checked:text-indigo-600 dark:peer-checked:text-indigo-300">Apply
                                                discount
