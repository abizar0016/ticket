...
class ApplyPromo
{
    public function handle(string $token, string $promoCode): JsonResponse
    {
        ...
        if (!$promo) {
            return response()->json(['success' => false, 'message' => 'Kode promo tidak ditemukan atau telah kadaluarsa'], 400);
        }
        ...
        if ($promo->is_ticket && !$promo->is_merchandise && $ticketQty === 0) {
            return response()->json(['success' => false, 'message' => 'Promo ini hanya untuk tiket, namun keranjang tidak berisi tiket'], 400);
        }

        if ($promo->is_merchandise && !$promo->is_ticket && $merchQty === 0) {
            return response()->json(['success' => false, 'message' => 'Promo ini hanya untuk merchandise, namun keranjang tidak berisi merchandise'], 400);
        }

        if (($promo->is_ticket && $promo->is_merchandise) && ($ticketQty === 0 && $merchQty === 0)) {
            return response()->json(['success' => false, 'message' => 'Promo ini membutuhkan setidaknya 1 tiket atau merchandise di keranjang'], 400);
        }

        $productsEventIds = $products->pluck('event_id')->unique();
        if ($productsEventIds->count() > 1 || !$productsEventIds->contains($promo->event_id)) {
            return response()->json(['success' => false, 'message' => 'Promo tidak valid untuk event yang dipilih'], 400);
        }

        if ($promo->min_tickets && $ticketQty < $promo->min_tickets) {
            return response()->json(['success' => false, 'message' => "Promo ini membutuhkan minimal {$promo->min_tickets} tiket"], 400);
        }

        if ($promo->min_merchandise && $merchQty < $promo->min_merchandise) {
            return response()->json(['success' => false, 'message' => "Promo ini membutuhkan minimal {$promo->min_merchandise} merchandise"], 400);
        }
